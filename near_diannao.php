<?php
    ...
    $sql = "select count(*) from near_week_index_detail where problemtype = '电脑'";
    $result = $db->query($sql);
    $sj = $result->fetch_row();
    $total = $sj[0];
    $page = new Page($total,20,"",true);
    $sqlshow = "select * from near_week_index_detail where problemtype = '电脑' ".$page->limit;
    $resultall = $db->query($sqlshow);
    }
    else
    {
    $cityname = $_SESSION['cityname'];
    $sql = "select count(*) from near_week_index_detail where problemtype = '电脑' AND CityName = '$cityname' ";
    $result = $db->query($sql);
    $sj = $result->fetch_row();
    $total = $sj[0];
    $page = new Page($total,20,"",true);
    $sqlshow = 
    "select * from near_week_index_detail where problemtype = '电脑' AND CityName = '$cityname' ".$page->limit;
    $resultall = $db->query($sqlshow);
    }
