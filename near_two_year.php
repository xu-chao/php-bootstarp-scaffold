...
FROM problemdetail
WHERE year(problemdetail.PDTime)=year(date_sub(now(),interval 2 year))
...
SELECT SUBSTRING(PDTime, 6, 2) AS month,COUNT(PDID) AS mount
FROM problemdetail
WHERE year(problemdetail.PDTime)=year(date_sub(now(),interval 2 year)) AND CityName = '$cityname'
GROUP BY SUBSTRING(PDTime, 6, 2)
UNION
SELECT '01' as month,'0' as mount
union  
